...
RegisterNetEvent('inventory:server:OpenInventory', function(name, id, other)
	...
	if not ply.state.inv_busy then
		if name and id then
			...
			TriggerClientEvent("levi-inventory:client:closeinv", id)
...
RegisterNetEvent('levi-inventory:server:SaveStashItems', function(stashId, items)
    MySQL.insert('INSERT INTO stashitems (stash, items) VALUES (:stash, :items) ON DUPLICATE KEY UPDATE items = :items', {
        ['stash'] = stashId,
        ['items'] = json.encode(items)
    })
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot) then
			if AddItem(target, item.name, amount, false, item.info, item.created) then
				...
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
				TriggerClientEvent('levi-inventory:client:giveAnim', src)
				TriggerClientEvent('levi-inventory:client:giveAnim', target)
			else
...
QBCore.Functions.CreateCallback('levi-inventory:server:GetStashItems', function(_, cb, stashId)
